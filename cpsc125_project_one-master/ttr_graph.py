...
def find_path(board, first_city, second_city):
  ...
  #take first city, find all cities connected
  #store connections for each
  #if city = second city, return true
  #for each city connected, find city connected until second city is found

  if board[first_city] == set():
    return False

  connectedCities = []
  checkedCities= []
  for firstCityConnected in board[first_city]:
      connectedCities.append(firstCityConnected)
  

#   while(len(connectedCities) != 0):
      for cityBeingTested in connectedCities:
          if cityBeingTested == second_city:
              return True ##ENDS METHOD IN THEORY
          for cityConnectedToTester in board[cityBeingTested]:
              if not cityConnectedToTester in checkedCities:
                  connectedCities.append(cityConnectedToTester)
          checkedCities.append(cityBeingTested)
...
print_trains_and_tickets(trains, tickets, board)
